...
def schedule_tasks(tasks):
    ...
    dependencies = build_dependency_map(tasks)
    dependents_count = count_tasks_depending_on_each(tasks)  # ✅ compute once
    ...
    while total_tasks > 0:
        ...
        if queue_size > max_ready:
            max_ready = queue_size
        
        
        current_ready_tasks = []
        while not readyQueue.empty():
            current_ready_tasks.append(readyQueue.get())

        
        current_ready_tasks.sort(
            key=lambda tname: dependents_count.get(tname, 0), reverse=True
        )

        
        for t_id in current_ready_tasks:
            ...
            if not processor_found:
                # Put back if no processor available but exists
...
if __name__ == "__main__":
    ...
    print(f"Lower Bound (Critical Path) = {lb2:.2f}")
